...
const Ebfer = ({ layoutSource }: ProjectComponentProps) => {
  const titleLayoutId = `${layoutSource === 'home' ? 'home' : 'list'}-title-ebfer`;
      ...
      <div className="w-full max-w-[76rem] h-auto lg:max-h-[85vh] lg:overflow-y-auto lg:no-scrollbar pt-8 lg:pt-0 pb-24 lg:pb-20">
        ...
        <div className="pb-6 mb-8 relative">
          ...
          <div className="flex items-baseline justify-between gap-4 flex-wrap mb-3">
            ...
            <span className="text-xs text-muted font-mono">c / seccomp-bpf / 2026</span>
          </div>
          ...
          >
            <a
              href="https://github.com/prathamanvekar/kernel-sandbox"
              target="_blank"
            ...
            >
              ...
              <span>prathamanvekar/kernel-sandbox</span>
        ...
        >
          a policy-driven c sandboxing engine using seccomp-bpf filters.
        </motion.p>
        ...
        >
          <p>ebfer is a kernel-level sandboxing engine written in c that uses seccomp-bpf (berkeley packet filter) to restrict the system calls a process is allowed to make at runtime.</p>
          <p>policies are defined declaratively and compiled to bpf bytecode, then attached to child processes via fork/exec. any syscall outside the allowlist is blocked by the kernel before it executes, eliminating a full class of privilege escalation and container escape attacks.</p>
          <p>runtime violations are logged with full context — pid, syscall number, arguments — making forensic analysis straightforward without needing kernel modules or elevated privileges on the host.</p>
        </motion.div>
        ...
        >
          <div className="flex items-center justify-between px-4 py-2 border-b border-border/10 bg-border/5">
            <span className="text-[10px] text-muted tracking-wider uppercase font-semibold">sandbox runtime workflow</span>
          ...
          <div className="p-5 md:p-6 text-muted/95 font-mono select-none overflow-x-auto">
            <div className="flex flex-col gap-4 max-w-3xl min-w-[580px]">
              ...
              <div className="flex items-center pl-6 text-muted/30 gap-2">
                ...
                <span className="text-[10px] text-muted/40 uppercase tracking-widest font-semibold">syscall filter</span>
              </div>

              {/* Step 2: Policy Evaluation Interceptor */}
              <div className="px-2.5 py-1.5 rounded border border-border/40 bg-white/[0.01] text-text font-medium self-start">
                [seccomp-bpf]
              </div>

              {/* Step 3: Branch Outcomes (Dashed Left-Border Tree Line) */}
              <div className="flex flex-col gap-3 pl-4 md:pl-6 border-l border-dashed border-border/20 ml-2">
                
                {/* Branch A: Block Path */}
                <div className="flex flex-row flex-nowrap items-center gap-3 whitespace-nowrap">
                  <span className="text-muted/30 select-none text-sm font-light">└──</span>
                  <div className="px-2.5 py-1 rounded-full border border-red-500/20 bg-red-500/5 text-red-400 text-xs font-medium">
                    (block)
                  </div>
                  
                  <span className="text-muted/30 font-light">→</span>
                  
                  <div className="px-2.5 py-1.5 rounded border border-red-500/20 bg-red-500/5 text-red-400 font-bold shadow-sm shadow-red-500/5">
                    [violation log]
                  </div>
                </div>
                ...
                <div className="flex flex-row flex-nowrap items-center gap-3 whitespace-nowrap">
                  <span className="text-muted/30 select-none text-sm font-light">└──</span>
                  <div className="px-2.5 py-1 rounded-full border border-emerald-500/20 bg-emerald-500/5 text-emerald-400 text-xs font-medium">
                    (allow)
                  </div>
