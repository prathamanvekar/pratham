...
const Ergen = ({ layoutSource }: ProjectComponentProps) => {
  const titleLayoutId = `${layoutSource === 'home' ? 'home' : 'list'}-title-ergen`;
    ...
    >
      <div className="w-full max-w-[76rem] h-auto lg:max-h-[85vh] lg:overflow-y-auto lg:no-scrollbar pt-8 lg:pt-0 pb-24 lg:pb-20">
        ...
        <div className="pb-6 mb-8 relative">
          ...
          >
            <a
              href="https://github.com/prathamanvekar/ergen"
              target="_blank"
            ...
            >
              ...
              <span>prathamanvekar/ergen</span>
        ...
        >
          an ast driven local first tool for automating go\'s repetitive error handling.
        </motion.p>
        ...
        >
          <p>ergen is a code generation tool that parses go source files using the go/ast package to identify functions that return errors, then automatically scaffolds idiomatic error handling boilerplate inline.</p>
          <p>it supports custom error templates on a per-project basis, letting teams define their own error wrapping conventions — useful for systems that need consistent context propagation or specific logging patterns at every call site.</p>
          <p>the tool runs entirely locally with no external dependencies, making it safe to integrate into any ci pipeline or pre-commit hook without network access.</p>
        </motion.div>
        ...
        >
          <div className="flex items-center justify-between px-4 py-2 border-b border-border/10 bg-border/5">
            <span className="text-[10px] text-muted tracking-wider uppercase font-semibold">ast parsing workflow</span>
          ...
          <div className="p-5 md:p-6 text-muted/95 font-mono select-none overflow-x-auto">
            <div className="flex flex-col gap-4 max-w-3xl min-w-[580px]">
              ...
              <div className="flex flex-row flex-nowrap items-center gap-2 md:gap-3 whitespace-nowrap">
                ...
                <div className="px-2.5 py-1.5 rounded border border-border/30 bg-white/[0.01] text-muted/80">
                  [ast inspection]
                </div>
              </div>

              {/* Bulletproof Downward Connector (Stays in perfect flow) */}
              <div className="flex items-center pl-6 text-muted/30 gap-2 animate-pulse">
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M19 14l-7 7m0 0l-7-7m7 7V3" />
                </svg>
                <span className="text-[10px] text-muted/40 uppercase tracking-widest font-semibold">codegen pipeline</span>
              </div>
              ...
              <div className="flex flex-row flex-nowrap items-center gap-2 md:gap-3 whitespace-nowrap">
                ...
                <div className="px-2.5 py-1 rounded-full border border-accent/20 bg-accent/5 text-accent text-xs font-medium">
                  (scaffold inline)
                </div>
