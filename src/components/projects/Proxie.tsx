...
const Proxie = ({ layoutSource }: ProjectComponentProps) => {
  const titleLayoutId = `${layoutSource === 'home' ? 'home' : 'list'}-title-proxie`;
      ...
      <div className="w-full max-w-[76rem] h-auto lg:max-h-[85vh] lg:overflow-y-auto lg:no-scrollbar pt-8 lg:pt-0 pb-24 lg:pb-20">
        ...
        <div className="pb-6 mb-8 relative">
          ...
          <div className="flex items-baseline justify-between gap-4 flex-wrap mb-3">
            ...
            <span className="text-xs text-muted font-mono">python / gemma / react / 2026</span>
          </div>
          ...
          >
            <a
              href="https://github.com/prathamanvekar/secure-llm-proxy"
              target="_blank"
            ...
            >
              ...
              <span>prathamanvekar/secure-llm-proxy</span>
        ...
        >
          a security-first gateway that secures ai workflows and local quantized gemma 3 inference.
        </motion.p>
        ...
        >
          <p>proxie is a reverse proxy layer that sits between client applications and a locally hosted gemma 3 model, adding a threat detection pipeline before any prompt reaches the llm.</p>
          <p>incoming requests are scanned for pii (names, emails, card numbers) using regex and ner models, checked against a prompt injection signature database, and flagged for anomalous intent patterns. requests that trip any threshold are blocked and logged before inference begins.</p>
          <p>the system exposes a react dashboard for monitoring flagged requests, reviewing telemetry, and adjusting detection sensitivity in real time — making it practical for teams evaluating local ai deployments in regulated environments.</p>
        </motion.div>
        ...
        >
          <div className="flex items-center justify-between px-4 py-2 border-b border-border/10 bg-border/5">
            <span className="text-[10px] text-muted tracking-wider uppercase font-semibold">proxy runtime workflow</span>
          ...
          <div className="p-5 md:p-6 text-muted/95 font-mono select-none overflow-x-auto">
            <div className="flex flex-col gap-4 max-w-3xl min-w-[580px]">
              ...
              <div className="flex items-center pl-6 text-muted/30 gap-2">
                ...
                <span className="text-[10px] text-muted/40 uppercase tracking-widest font-semibold">gateway validation</span>
              </div>
              ...
              <div className="flex flex-row flex-nowrap items-center gap-2 md:gap-3 whitespace-nowrap">
                ...
                <div className="px-2 py-1 rounded border border-red-500/20 bg-red-500/5 text-red-400 text-xs">
                  (sanitize PII & block injection)
                </div>
              </div>
              ...
              <div className="flex items-center pl-6 text-muted/30 gap-2">
                ...
                <span className="text-[10px] text-muted/40 uppercase tracking-widest font-semibold">inference pipeline</span>
              </div>

              {/* Step 3: Local LLM Output */}
              <div className="flex flex-row flex-nowrap items-center gap-2 md:gap-3 whitespace-nowrap">
                <div className="px-2.5 py-1.5 rounded border border-border/40 bg-white/[0.01] text-text font-medium">
                  [gemma 3 model]
                </div>
